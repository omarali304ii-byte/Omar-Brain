...
description: External workflow specialist for APIs, webhooks, retries, idempotency, uncertain outcomes and reconciliation.
...
hooks:
  PreToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: 'node "00_System/Project Council OS/runtime/validate-specialist-write.mjs" "Integration and Workflow"'
    - matcher: "Bash"
      hooks:
        - type: command
          command: 'node "00_System/Project Council OS/runtime/validate-specialist-bash.mjs" "Integration and Workflow"'
type: agent-role
status: active
created: 2026-07-09
updated: 2026-07-09
...
ai_access: allowed
role_id: agent-claude-integration-workflow-agent
---
...
2. If `20_Agent_Council/` exists, locate `Agents/Integration and Workflow/`.
...
## Output shape
```yaml
agent: integration-workflow-agent
project:
repo_revision:
freshness:
scope_inspected:
