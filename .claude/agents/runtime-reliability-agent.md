...
description: Runtime specialist for workers, deployment, recovery, observability, CI/CD, reproducibility and operational proof.
...
hooks:
  PreToolUse:
    - matcher: "Write|Edit"
      hooks:
        - type: command
          command: 'node "00_System/Project Council OS/runtime/validate-specialist-write.mjs" "Runtime and Reliability"'
    - matcher: "Bash"
      hooks:
        - type: command
          command: 'node "00_System/Project Council OS/runtime/validate-specialist-bash.mjs" "Runtime and Reliability"'
type: agent-role
status: active
created: 2026-07-09
updated: 2026-07-09
...
ai_access: allowed
role_id: agent-claude-runtime-reliability-agent
---
...
2. If `20_Agent_Council/` exists, locate `Agents/Runtime and Reliability/`.
...
## Output shape
```yaml
agent: runtime-reliability-agent
project:
repo_revision:
freshness:
scope_inspected:
